...
const insertJob = (schedule, nextRunAtVal, enabled = 1) => {
  const r = db.prepare(
    'INSERT INTO jobs (user_id, key_id, name, schedule, tz, title, body, enabled, next_run_at, created_at, updated_at) VALUES (1,1,?,?,?,?,?,?,?,?,?)'
  ).run('任务', schedule, '+08:00', '', schedule, enabled, nextRunAtVal, now, now);
...
ck('推送到 DO', pushes.length === 1 && pushes[0].key_name === '告警通道', pushes[0]?.body);
...
// 5) 一次性任务执行后自动停用
const id5 = insertJob('once:2026-09-10T09:00', now - 5000);
