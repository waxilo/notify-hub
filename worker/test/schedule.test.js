...
// 秒对齐：创建时刻与执行时刻的「秒」都不参与计划，计划时刻一律落在整分钟。
// 21:11:30 创建的 every:1m，计划必须是 21:12:00 —— 否则 21:12:10 那次 tick 会因
// 「计划 21:12:30 还没到」被判为未到期，整条提醒晚一分钟。
let ns = nextRunAt('every:1m', OFF, Date.UTC(2026, 8, 10, 13, 11, 30), Date.UTC(2026, 8, 10, 13, 11, 30));
ck('every:1m 创建时刻带秒 -> 对齐到下一整分钟', f(ns) === '2026-09-10 21:12' && ns % 60000 === 0, f(ns));

ns = nextRunAt('every:1m', OFF, Date.UTC(2026, 8, 10, 13, 12, 10), Date.UTC(2026, 8, 10, 13, 12, 0));
ck('every:1m tick 晚 10 秒不把秒带进下一次', f(ns) === '2026-09-10 21:13' && ns % 60000 === 0, f(ns));

ns = nextRunAt('every:1m', OFF, Date.UTC(2026, 8, 10, 13, 12, 10), Date.UTC(2026, 8, 10, 13, 11, 0));
ck('every:1m 计划 21:12 + tick 21:12:10 -> 21:13', f(ns) === '2026-09-10 21:13', f(ns));

// 遗留数据（改动前建的周期任务，prev 带 30 秒）下次触发即自愈，不残留半分钟偏移
ns = nextRunAt('every:5m', OFF, Date.UTC(2026, 8, 10, 13, 21, 0), Date.UTC(2026, 8, 10, 13, 20, 30));
ck('遗留带秒 prev 自愈为整分钟槽位', f(ns) === '2026-09-10 21:25' && ns % 60000 === 0, f(ns));
